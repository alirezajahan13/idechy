<?php
/**
 * idechy functions and definitions
 *
 ...
 */
function idechy_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on idechy, use a find and replace
		* to change 'idechy' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'idechy', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'idechy' ),
		)
	);
...
add_action( 'after_setup_theme', 'idechy_setup' );
 ...
 */
function idechy_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'idechy_content_width', 640 );
}
add_action( 'after_setup_theme', 'idechy_content_width', 0 );
 ...
 */
function idechy_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'idechy' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'idechy' ),
...
add_action( 'widgets_init', 'idechy_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'idechy_scripts' );
